...
class LaTeX2VideoGUI:
    """Main GUI application for LaTeX2Video"""
    def __init__(self, root):
        self.root = root
        self.root.title("LaTeX2Video")
    ...
    def create_ui(self):
        ...
        config_entry = ttk.Entry(top_frame, textvariable=self.config_file_path, width=50)
        config_entry.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=5)
        ...
        output_entry = ttk.Entry(top_frame, textvariable=self.output_dir, width=50)
        output_entry.grid(row=2, column=1, sticky=tk.EW, padx=5, pady=5)
    ...
    def generate_images(self):
        ...
        threading.Thread(target=self._generate_images_thread, args=(latex_file,), daemon=True).start()

    def _generate_images_thread(self, latex_file):
        """Thread function for generating images"""
        try:
            # Add the LaTeX file path to the configuration
            config_copy = self.config.copy()  # Create a copy to avoid thread issues
            config_copy['latex_file_path'] = os.path.abspath(latex_file)
            
            # Generate slide images
            image_paths = generate_slide_images(latex_file, config_copy)
            
            # Use a safer way to update the UI from a thread
            def update_ui():
                if not image_paths:
    ...
    def generate_audio(self):
        ...
        # Run in a separate thread to avoid freezing the UI
        threading.Thread(target=self._generate_audio_thread, daemon=True).start()
    ...
    def _generate_audio_thread(self):
        ...
        try:
            ...
            def show_error():
                messagebox.showerror("Error", f"Failed to generate audio: {error_msg}")
    ...
    def assemble_video(self):
        ...
        # Run in a separate thread to avoid freezing the UI
        threading.Thread(target=self._assemble_video_thread, args=(slides_dir, audio_dir), daemon=True).start()

    def _assemble_video_thread(self, slides_dir, audio_dir):
        """Thread function for assembling video"""
        try:
            ...
            # Assemble video
            output_path = assemble_video(image_files, audio_files, config_copy)
            ...
            def show_error():
                messagebox.showerror("Error", f"Failed to assemble video: {error_msg}")
    ...
    def generate_all(self):
        ...
        # Run in a separate thread to avoid freezing the UI
        threading.Thread(target=self._generate_all_thread, args=(latex_file,), daemon=True).start()

    def _generate_all_thread(self, latex_file):
        """Thread function for generating everything"""
        try:
            # Make a copy of the data to avoid thread issues
            config_copy = self.config.copy()
            narrations_copy = self.narrations.copy()
            ...
            # 1. Generate slide images - update UI first
            def update_status_step1():
                self.update_status("Step 1: Generating slide images...")
            
            self.root.after(0, update_status_step1)
            
            # Wait a bit to ensure UI update happens
            import time
            time.sleep(0.1)
            ...
            def update_ui_after_images():
                ...
                # Continue with step 2 in the main thread
                self.root.after(100, lambda: continue_with_step2(image_paths))
            
            # Schedule UI update
            self.root.after(0, update_ui_after_images)
            
            # Function to continue with step 2
            def continue_with_step2(image_paths):
                # Update status for step 2
                self.update_status("Step 2: Generating audio files...")
                
                # Schedule the actual audio generation
                self.root.after(100, lambda: generate_audio_step(image_paths))
            
            # Function to handle audio generation
            def generate_audio_step(image_paths):
                try:
                    # Generate audio files
                    audio_paths = generate_all_audio(narrations_copy, config_copy)
                    
                    # Schedule UI update
                    self.root.after(0, lambda: after_audio_generation(image_paths, audio_paths))
                except Exception as e:
                    logging.error(f"Error generating audio: {e}")
                    error_msg = str(e)
            ...
            # Function to handle after audio generation
            def after_audio_generation(image_paths, audio_paths):
                if not audio_paths:
                    messagebox.showerror("Error", "Failed to generate audio files.")
                    self.update_status("Failed to generate audio files.")
                    return
                ...
                self.root.after(100, lambda: continue_with_step3(image_paths, audio_paths))
            
            # Function to continue with step 3
            def continue_with_step3(image_paths, audio_paths):
                # Update status for step 3
                ...
                # Schedule the actual video assembly
                self.root.after(100, lambda: assemble_video_step(image_paths, audio_paths))
            
            # Function to handle video assembly
            def assemble_video_step(image_paths, audio_paths):
                try:
                    # Assemble video
                    output_path = assemble_video(image_paths, audio_paths, config_copy)
                    
                    # Schedule UI update
                    self.root.after(0, lambda: after_video_assembly(output_path))
                except Exception as e:
                    logging.error(f"Error assembling video: {e}")
                    error_msg = str(e)
            ...
            # Function to handle after video assembly
            def after_video_assembly(output_path):
                if not output_path:
                    messagebox.showerror("Error", "Failed to assemble video.")
                    self.update_status("Failed to assemble video.")
                    return
            ...
            # Note: The actual execution flow is controlled by the scheduled after() calls
            ...
            def show_error():
                messagebox.showerror("Error", f"Failed to complete video generation: {error_msg}")
