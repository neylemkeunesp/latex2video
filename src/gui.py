...
class LaTeX2VideoGUI:
    ...
    def _generate_images_thread(self, latex_file):
        ...
        try:
            ...
            if not image_paths:
                self.root.after(0, lambda: messagebox.showerror("Error", "Failed to generate slide images."))
                self.root.after(0, lambda: self.update_status("Failed to generate slide images."))
                return
            
            self.root.after(0, lambda: self.update_status(f"Generated {len(image_paths)} slide images."))
            self.root.after(0, lambda: messagebox.showinfo("Success", f"Generated {len(image_paths)} slide images."))
    ...
    def _generate_audio_thread(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self.update_status(f"Generated {len(audio_paths)} audio files."))
            self.root.after(0, lambda: messagebox.showinfo("Success", f"Generated {len(audio_paths)} audio files."))
    ...
    def _assemble_video_thread(self, slides_dir, audio_dir):
        ...
        try:
            ...
            self.root.after(0, lambda: messagebox.showerror("Error", f"Failed to assemble video: {e}"))
            self.root.after(0, lambda: self.update_status("Error assembling video."))
    ...
    def _generate_all_thread(self, latex_file):
        ...
        try:
            ...
            self.root.after(0, lambda: self.update_status("Step 1: Generating slide images..."))
            image_paths = generate_slide_images(latex_file, self.config)
            
            if not image_paths:
                self.root.after(0, lambda: messagebox.showerror("Error", "Failed to generate slide images."))
                self.root.after(0, lambda: self.update_status("Failed to generate slide
