#!/usr/bin/env python3
"""
Helper script to run the LaTeX2Video GUI in WSL or environments with X11 issues.
This script avoids using XInitThreads and threading entirely.
"""
...
# Disable threading in Tkinter/X11
...
class LaTeX2VideoGUI:
    """Main GUI application for LaTeX2Video - WSL Compatible Version"""
    def __init__(self, root):
        self.root = root
        self.root.title("LaTeX2Video (WSL Compatible)")
    ...
    def create_ui(self):
        ...
        # Create Entry widgets separately to avoid XCB issues
        latex_entry = ttk.Entry(top_frame, textvariable=self.latex_file_path, width=50)
        latex_entry.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)
    ...
    def generate_images(self):
        """Generate images from the LaTeX file - WSL Compatible Version"""
        ...
        try:
            ...
            # Generate slide images - this will block the UI but avoid threading issues
    ...
    def generate_audio(self):
        """Generate audio files from narration scripts - WSL Compatible Version"""
        if not self.load_config():
            return
        ...
        try:
            ...
            # Generate audio files - this will block the UI but avoid threading issues
    ...
    def assemble_video(self):
        """Assemble the final video from images and audio - WSL Compatible Version"""
        if not self.load_config():
            return
        ...
        try:
            ...
            # Assemble video - this will block the UI but avoid threading issues
    ...
    def generate_all(self):
        """Generate everything: images, audio, and video - WSL Compatible Version"""
        if not self.load_config():
            return
...
def main():
    ...
    # Try to run the GUI
    try:
        print("Attempting to run LaTeX2Video GUI (WSL Compatible Version)...")
